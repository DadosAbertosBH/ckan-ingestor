...
- **`ingestor_orchestrator`** — API de leitura (FastAPI), frontend (Vue 3) e serviço Go para Iggy, escrita, scheduler e consumo de resultados.
...
```mermaid
graph TD
    UI[Frontend Vue.js] -->|leitura HTTP| API[API FastAPI]
    UI -->|escrita HTTP :8081| GO[Serviço Go]
    API -->|SQL| MYSQL[(MySQL)]
    GO -->|SQL| MYSQL
    GO -->|publica comandos| IGGY{{Apache Iggy}}
    IGGY -->|consome jobs| WORKER[Worker Rust]
    WORKER -->|ingestão de dados| LAKE
    WORKER -->|publica resultados| IGGY
    IGGY -->|consome resultados| GO
...
|---|---|---|
| API | FastAPI + SQLAlchemy async | Endpoints REST de consulta para jobs, datasets, instâncias, sync e dashboard |
...
|---|---|---|
| `jobs` | coordinator → worker | Jobs de ingestão |
| `jobs-retry` | serviço Go → worker | Jobs com retry |
| `job-results` | worker → serviço Go | Resultados da ingestão |
| `ckan_metadata_sync` | serviço Go → worker-coordinator | Comandos de sincronização de metadados CKAN |
| `ckan_metadata_sync_result` | worker-coordinator → serviço Go | Resultados da sincronização de metadados |
...
│   ├── backend/                   # API FastAPI de leitura
│   ├── frontend/                  # Dashboard Vue.js
│   └── go/                        # Serviço Go de orquestração/Iggy
...
| API de escrita Go | http://localhost:8081 |
...
| `INGEST_ORCH_IGGY_ADDRESS` | `localhost:8090` | Endpoint TCP do Iggy, usado pelo serviço Go |

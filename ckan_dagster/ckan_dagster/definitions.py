...
from ckan_dagster.ckan_dagster.resources.ckan_fetcher_resource import CkanFetcherResource
from ckan_dagster.ckan_dagster.resources.duckdb_data_ingestor_resource import DuckdbDataIngestorResource
...
def ckan_datasets(
    ...
    preview_df = (metadata_ingestor.conn.execute("select id, name, url, metadata_modified from ckan_dataset limit 10")
                  .df()
...
def ckan_resources(
    ...
    count = (metadata_ingestor.conn.execute("select count(*) from ckan_resource").fetchone())[0]
    return dg.MaterializeResult(
        metadata={
            "row_count": dg.MetadataValue.int(count),
            "preview": dg.MetadataValue.md(
                metadata_ingestor.conn.execute("select id, name, url, last_modified from ckan_resource limit 10")
                .fetchdf()
                .to_markdown(index=False)
            ),
        }
...
def ckan_data(context: dg.AssetExecutionContext, ingestor: dg.ResourceParam[DuckdbCkanDataIngestor]):
    resource_id = context.partition_key
    resource = (ingestor.ducklake_conn.execute(
        "select * from ckan_resource where id = ?",
        (resource_id,)
    ).arrow().to_pylist()[0])
...
def ckan_data_request_sensor(context: dg.SensorEvaluationContext,
                             metadata_ingestor: dg.ResourceParam[DuckdbCkanMetadataIngestor]):
    skipped = []
    requested = []
    resources = metadata_ingestor.conn.execute("select id, last_modified from ckan_resource").fetchall()
    resource_ids = [r[0] for r in resources]
    for r in resources:
        r_id, r_last_modified = r
        if metadata_ingestor.table_is_up_to_date(r_id, r_last_modified):
            skipped.append(dg.SkipReason(f"Asset {r_id} is up to date, last_mofied = {r_last_modified}"))
        else:
            requested.append(dg.RunRequest(
    ...
    return dg.SensorResult(
        ...
        dynamic_partitions_requests=[resource_partitions.build_add_request(resource_ids)]
    )

...
class TestApplyResultLabels:
    async def test_csv_delimiter_is_upserted_as_a_csv_hint(
        self, db_session, default_instance
    ):
        job = CkanDataJob(
            resource_id="r-csv-hint",
            dataset_name="ds-csv-hint",
            idempotency_key="r-csv-hint",
            instance_id=default_instance.id,
            status=JobStatus.PENDING,
        )
        db_session.add(job)
        ...
        data.update({"job_id": job.id, "csv_delimiter": ";"})
        await service.apply_result(data)

        data["csv_delimiter"] = ","
        await service.apply_result(data)

        hint = await db_session.get(CsvHint, job.resource_id)
        assert hint is not None
        assert hint.delimiter == ","

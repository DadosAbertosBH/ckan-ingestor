...
func (f *fakeDriver) EnsureStream(value string) error {
	f.streams = append(f.streams, value)
	return nil
}
func (f *fakeDriver) EnsureTopic(value string, count int) error {
	if f.topics == nil {
...
func (f *fakeDriver) Send(topic string, partition int, payload []byte) error {
...
func (f *fakeDriver) Join(string, string) error { return nil }
func (f *fakeDriver) Poll(string, string, int) ([]Message, error) {
	values := f.messages
	f.messages = nil
	return values, nil
}
func (f *fakeDriver) Commit(_ string, _ string, partition uint32, offset uint64) error {
